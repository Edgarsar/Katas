...
//   return /^[-+]?([0-8]?\d(\.\d+)?|90(\.0+)?),\s*[-+]?(180(\.0+)?|((1[0-7]\d)|([1-9]?\d))(\.\d+)?)$/g.test(coordinates);
// }
...
function isValidCoordinates(coordinates) {
  ...
  if (lat.includes("e") || long.includes("e")) {
    return false;
  }
...
const ValidCoordinates = [
  ...
  "04, -23.234235",
  "43.91343345, 143",
  "75.08014520050006, 311.8614561673996"
];
for (i in ValidCoordinates) {
  console.log(isValidCoordinates(ValidCoordinates[i]), ValidCoordinates[i] + " validation failed.");
}
...
for (i in InvalidCoordinates) {
  console.log(!isValidCoordinates(InvalidCoordinates[i]), InvalidCoordinates[i] + " validation failed.");
}