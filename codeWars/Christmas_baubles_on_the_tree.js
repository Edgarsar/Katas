...
function baublesOnTree(baubles, branches) {
  ...
  for (let i = 0; i < branches; i++) {
    if (i < baubles % branches)
      arr.push(Math.ceil(baubles / branches));
    else
      arr.push(Math.floor(baubles / branches));
...
console.log(baublesOnTree(5, 0))//, "Grandma, we will have to buy a Christmas tree first!");
